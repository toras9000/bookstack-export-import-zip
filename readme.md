...
It is made in a C# file-based apps that runs in .NET10.  
...
The following installations are required to run C# file-based apps  
...
```
dotnet run --file <target-script-file>
```

If the execution environment is Windows, the auxiliary script for file association is also included in `modules/@windows-associate.cs1`.  
